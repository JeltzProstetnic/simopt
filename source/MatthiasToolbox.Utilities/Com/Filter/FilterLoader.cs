...
namespace MatthiasToolbox.Utilities.COM.Filter
{
    /// <summary>
    ...
    static class FilterLoader
    {
        ...
        private static bool GetFilterDllAndClassFromPersistentHandler(string persistentHandlerClass, out string dllName, out string filterPersistClass)
        {
            ...
            if (String.IsNullOrEmpty(filterPersistClass)) return false;

            //Read the dll name 
            dllName = ReadStrFromHKLM(@"Software\Classes\CLSID\" + filterPersistClass + @"\InprocServer32");
        ...
        private static string GetPersistentHandlerClass(string ext, bool searchContentType)
        {
            ...
            if (String.IsNullOrEmpty(persistentHandlerClass)) // try getting the info from the document type 
                persistentHandlerClass = GetPersistentHandlerClassFromDocumentType(ext);
            
            if (searchContentType && String.IsNullOrEmpty(persistentHandlerClass)) // try getting the info from the Content Type

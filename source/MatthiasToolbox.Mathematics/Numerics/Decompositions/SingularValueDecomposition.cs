...
namespace MatthiasToolbox.Mathematics.Numerics.Decompositions
{
        ...
        public SingularValueDecomposition(double[,] value, bool computeLeftSingularVectors, bool computeRightSingularVectors)
        {
            ...
            for (int k = 0; k < System.Math.Max(nct, nrt); k++)
            {
                if (k < nct)
                {
                    ...
                    for (int i = k; i < m; i++)
                    {
                        s[k] = MMath.Hypotenuse(s[k], a[i,k]);
                    }
                    ...
                    case 1:
                        {
                            ...
                            for (int j = p - 2; j >= k; j--)
                            {
                                double t = MMath.Hypotenuse(s[j], f);
                                double cs = s[j] / t;
                                double sn = f / t;
                                s[j] = t;
                                if (j != k)

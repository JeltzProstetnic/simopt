...
namespace MatthiasToolbox.Mathematics.Numerics
{
    /// <summary>
    /// A class to comfortably manipulate complex numbers.
    /// 
    /// Parts are adapted from the AForge.NET framework by
    /// Andrew Kirillov (andrew.kirillov@aforgenet.com) and
    /// Israel Lot (israel.lot@gmail.com)
    /// (see http://www.aforgenet.com/framework/)
    /// </summary>
    /// <remarks>
    /// <para>The class encapsulates complex number and provides
    ...
    /// </code>
    /// </remarks>
    public struct Complex : ICloneable, ISerializableSimulation, IEquatable<Complex>
    {
        #region over
        ...
        /// <summary>
        /// Includes string representations of infinity (oo), the imaginary unit (i) and NaN (?)
        /// </summary>
        /// <returns></returns>
        public string ToStringElaborate()
        {
            ...
            if (double.IsNaN(this.Im)) iString = "?";
            else if (double.IsNegativeInfinity(this.Im)) iString = "-oo";
            else if (double.IsPositiveInfinity(this.Im)) iString = "oo";
            else iString = Im.ToString();

            if (double.IsNaN(this.Re)) rString = "?";
            else if (double.IsNegativeInfinity(this.Re)) rString = "-oo";
            else if (double.IsPositiveInfinity(this.Re)) rString = "oo";
            else rString = Re.ToString();

            if (this.IsImaginary) return iString + "i";
            else if (this.IsReal) return rString;
            else return "(" + rString + "," + iString + ")";
        }
        ...
        {
            return (Exp(2d * a) - 1d) / (Exp(2d * a) + 1d);
        }
        ...
        /// <summary>
        /// Divides a complex number by a scalar value. Division by zero yields double.NaN, no exception.
        /// </summary>
        /// 
        /// <param name="a">A <see cref="Complex"/> instance.</param>
        /// <param name="s">A scalar value.</param>
        ...
        /// <exception cref="DivideByZeroException">Can not divide by zero.</exception>
        /// 
        public static Complex DivideSafe(Complex a, double s)
        {
            return new Complex(a.Re / s, a.Im / s);
        }
        ...
        /// <summary>
        /// calculate a^b (a to the power of b)
        /// </summary>
        /// <param name="a"></param>
        /// <param name="b"></param>
        /// <returns></returns>
        ...
        public static Complex Sqrt(Complex a)
        {
            ...
            else
            {
                ...
                if (a.Im < 0.0) result.Im = -result.Im;
            }
        ...
        public static double Arg(Complex a)
        {
            if (a.Re == 0)
            {
                if (a.Im > 0) return Math.PI / 2;
                else if (a.Im < 0) return -Math.PI / 2;
        ...
        {
            return new Complex(Re / ((Re * Re) + (Im * Im)),
                -Im / ((Re * Re) + (Im * Im)));
        }
        ...
        /// </summary>
        /// <param name="a"></param>
        /// <returns>a new complex number with this' conjugated values</returns>
        ...
        {
            return other.Re.Equals(Re) && other.Im.Equals(Im);
        }
        ...
        /// <summary>
        /// calculate a^k (a to the power of k)
        /// CAUTION: this is NOT logical XOR but power!
        /// </summary>
        /// <param name="A"></param>
        /// <param name="k"></param>
        /// <returns></returns>
        ...
        /// <param name="k"></param>
        /// <returns></returns>
        public static Complex operator ^(Complex A, Complex k)
        {
            return Pow(A, k);
        }
        ...
        /// <summary>
        /// subtract 1 from the real part of this number
        /// </summary>
        /// <param name="a"></param>
        /// <returns></returns>
        public static Complex operator --(Complex a)
        ...
        /// <summary>
        /// add 1 to the real part of this number
        /// </summary>
        /// <param name="a"></param>
        /// <returns></returns>
        public static Complex operator ++(Complex a)
        ...
        public static Complex operator /(Complex a, double s)
        {
            return Complex.Divide(a, s); // TODO: switch to DivideSafe to allow division by zero.
        }
        ...
        public static implicit operator Complex(double[] value)
        {
            if (value == null) throw new ArgumentNullException("Complex is a struct and cannot be assigned null.");
            if (value.Length != 2) throw new InvalidCastException("Only arrays of size two can be cast to a complex number.");
            return new Complex(value[0], value[1]);
        }

        public static implicit operator Complex(decimal value)
        {
            return new Complex(decimal.ToDouble(value));
        }

        public static explicit operator double(Complex value)
        {
            if (!value.IsReal) throw new InvalidCastException("Cannot cast given complex number to non-complex because it's imaginary part != zero.");
            return value.Real;
        }
        ...
        {
            if (!a.IsReal || !b.IsReal) throw new InvalidOperationException("The size of complex numbers can only be compared if their imaginary part is zero.");
            return a.Re < b.Re;
        }

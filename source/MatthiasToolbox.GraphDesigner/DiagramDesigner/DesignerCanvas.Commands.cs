...
namespace MatthiasToolbox.GraphDesigner.DiagramDesigner
{
    public partial class DesignerCanvas
    {
        ...
        #endregion
        #region Open Command
        //TODO  GraphDesigner - dwi refactor
        ...
        private void BringForward_Executed(object sender, ExecutedRoutedEventArgs e)
        {
            List<UIElement> ordered = (from item in SelectionService.CurrentSelection
                                       orderby Canvas.GetZIndex((UIElement)item) descending
        ...
        private void AlignVerticalCenters_Executed(object sender, ExecutedRoutedEventArgs e)
        {
            ...
            if (selectedItems.Count() > 1)
            {
                double bottom = Canvas.GetTop(selectedItems.First()) + selectedItems.First().ActualHeight / 2;

                foreach (DesignerItem item in selectedItems)
                {
                    double delta = bottom - (Canvas.GetTop(item) + item.ActualHeight / 2);
                    foreach (DesignerItem di in SelectionService.GetGroupMembers(item))
                    {
                        Canvas.SetTop(di, Canvas.GetTop(di) + delta);
                    }
        ...
        private void AlignBottom_Executed(object sender, ExecutedRoutedEventArgs e)
        {
            ...
            if (selectedItems.Count() > 1)
            {
                double bottom = Canvas.GetTop(selectedItems.First()) + selectedItems.First().ActualHeight;

                foreach (DesignerItem item in selectedItems)
                {
                    double delta = bottom - (Canvas.GetTop(item) + item.ActualHeight);
                    foreach (DesignerItem di in SelectionService.GetGroupMembers(item))
                    {
                        Canvas.SetTop(di, Canvas.GetTop(di) + delta);
                    }
        ...
        private void AlignHorizontalCenters_Executed(object sender, ExecutedRoutedEventArgs e)
        {
            ...
            if (selectedItems.Count() > 1)
            {
                double center = Canvas.GetLeft(selectedItems.First()) + selectedItems.First().ActualWidth / 2;

                foreach (DesignerItem item in selectedItems)
                {
                    double delta = center - (Canvas.GetLeft(item) + item.ActualWidth / 2);
                    foreach (DesignerItem di in SelectionService.GetGroupMembers(item))
                    {
                        Canvas.SetLeft(di, Canvas.GetLeft(di) + delta);
                    }
        ...
        private void AlignRight_Executed(object sender, ExecutedRoutedEventArgs e)
        {
            ...
            if (selectedItems.Count() > 1)
            {
                double right = Canvas.GetLeft(selectedItems.First()) + selectedItems.First().ActualWidth;

                foreach (DesignerItem item in selectedItems)
                {
                    double delta = right - (Canvas.GetLeft(item) + item.ActualWidth);
                    foreach (DesignerItem di in SelectionService.GetGroupMembers(item))
                    {
                        Canvas.SetLeft(di, Canvas.GetLeft(di) + delta);
                    }
        ...
        private void DistributeHorizontal_Executed(object sender, ExecutedRoutedEventArgs e)
        {
            ...
            if (selectedItems.Count() > 1)
            {
                ...
                foreach (DesignerItem item in selectedItems)
                {
                    ...
                    right = Math.Max(right, Canvas.GetLeft(item) + item.ActualWidth);
        ...
        private void DistributeVertical_Executed(object sender, ExecutedRoutedEventArgs e)
        {
            ...
            if (selectedItems.Count() > 1)
            {
                ...
                foreach (DesignerItem item in selectedItems)
                {
                    ...
                    bottom = Math.Max(bottom, Canvas.GetTop(item) + item.ActualHeight);
                    sumHeight += item.ActualHeight;
                }
        ...
        //                              new XElement("SourceID", connection.SourceConnector.ParentDesignerItem.ID),
        //                              new XElement("SinkID", connection.SinkConnector.ParentDesignerItem.ID),
        //                              new XElement("SourceConnectorName", connection.SourceConnector.Name),
        //                              new XElement("SinkConnectorName", connection.SinkConnector.Name),
        ...
        //                                       where item.ID == connection.SourceConnector.ParentDesignerItem.ID
        //                                       select item).FirstOrDefault();

        //            DesignerItem sinkItem = (from item in selectedDesignerItems
        //                                     where item.ID == connection.SinkConnector.ParentDesignerItem.ID
        //                                     select item).FirstOrDefault();
        ...
        private void DeleteCurrentSelection()
        {
            ...
            {
                this.Children.Remove(connection); //the connection unloaded event will remove the connection from the designeritem
                OnEdgeRemoved(new EdgeEventArgs() { Edge = connection.Edge });
            }

            foreach (DesignerItem item in SelectionService.CurrentSelection.OfType<DesignerItem>())
            {
                //remove all attached connections
                ...
                this.OnDesigneritemRemoved(new DesignerItemEventArgs() { Item = item });
            }

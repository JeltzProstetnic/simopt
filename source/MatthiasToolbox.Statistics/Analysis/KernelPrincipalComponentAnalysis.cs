...
{
    using System;
    using MatthiasToolbox.Statistics.Kernels;
    ...
    public class KernelPrincipalComponentAnalysis : PrincipalComponentAnalysis
    {
        ...
        public override void Compute()
        {
            ...
            eigs = MMath.Sort(evals, eigs, new GeneralComparer(ComparerDirection.Descending, true));
        ...
        /// variables as columns and observations of each variable as rows.</param>
        /// <param name="components">The number of components to use in the transformation.</param>
        ...
        public double[,] Revert(double[,] data, int neighbors)
        {
            ...
            if (distance == null) throw new Exception(
            ...
            for (int p = 0; p < rows; p++)
            {
               ...
               // double[,] H = Matrix.Centering(nn);
                ...
                double[,] L = MMath.Diagonal(nn, svd.Diagonal);
                ...
                //    d02 = sum(Z.^2)';
                double[] d02 = MMath.Sum(MMath.ElementwisePower(Z, 2));


                // 7. Get the pre-image using z = -0.5*inv(Z')*(d2-d02)
                double[,] inv = MMath.PseudoInverse(Z.Transpose());
                /*
                for (int i = 0; i < inv.GetLength(0); i++)
                {
                    for (int j = 0; j < inv.GetLength(1); j++)
                    {
                        if (Double.IsNaN(inv[i, j]))
                            inv[i, j] = 0;
                    }
                }*/
                ...
                double[] x = (U.Multiply(z)).Add(MMath.Sum(X.Transpose()).Multiply(1.0 / nn));
        ...
        private double[,] centerKernel(double[,] K)
        {
            ...
            double[] M = MMath.Mean(K);
            double MM = MMath.Mean(M);

...
| **SIM-90** | **P1** | Entity identity destroyed at every server | Blocks every multi-stage MCP model, blocks end-to-end cycle time, and blocks the Jackson-network case of SIM-64. It is on the critical path of the next slice, not beside it. | The fix touches `Server`, which everything in the repo uses, and could disturb Ivotion's batch-count compensation. Arguably needs its own careful slot rather than being squeezed in. |

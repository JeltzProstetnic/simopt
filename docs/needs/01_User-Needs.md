...
| Beachhead | General discrete manufacturing, sold **through independent operations consultants** to SMB manufacturers. Re-affirmed as the *public positioning* on 2026-08-26 (D-06), with **forensic casework as the proving ground** — the owner is customer zero via his own Sachverständigen practice. | The Consultant is the primary buyer persona; the Plant Engineer is the primary *user*. R5 is not a marketing target but *is* the first real user, so Chapter 9b needs are validated earliest of all. **No material may claim court-admissibility until the admissibility literature is researched.** |

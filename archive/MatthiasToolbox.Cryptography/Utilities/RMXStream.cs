...
{
    /// <summary>
    /// A wrapper to prepend and XOR data to any stream
    /// used for rmx hashing as in Halevi 2007
    /// </summary>
    public class RMXStream : Stream
    {
        #region classvar
        ...
        public override long Seek(long offset, SeekOrigin origin)
        {
            ...
            if (pos < 0) return originalStream.Seek(offset, origin);
            if (pos < rnd.Length)
            {
            ...
            {
                rndIndex = rnd.Length - 1;
                return originalStream.Seek(offset - rnd.Length, origin);
            }
        }
        ...
        public override int Read(byte[] buffer, int offset, int count)
        {
            ...
            {
                while (rndIndex < rnd.Length && count > 0)
                {
                    buffer[rndIndex + offset] = rnd[rndIndex];
                    rndIndex += 1;
                    count -= 1;
                    ivBytesAdded += 1;
                }
            }
            int ivStart = (int)originalStream.Position%rnd.Length;
            int readBytes = originalStream.Read(buffer, offset + ivBytesAdded, count) + ivBytesAdded;
            xor(buffer, offset + ivBytesAdded, ivStart, readBytes - ivBytesAdded);
            return readBytes;
        }

        private void xor(byte[] buffer, int offset, int ivoffset, int count)
        {
            int i = offset;
            while(count > 0)
            {
                if(i >= buffer.Length) return;
                buffer[i] = (byte)(buffer[i] ^ rnd[ivoffset]);
                count -= 1;
                ivoffset = (ivoffset + 1) % rnd.Length;
                i += 1;
            }
        ...
        {
            get { return originalStream.Length + rnd.Length; }
        }
        ...
        {
            get { return rndIndex + originalStream.Position; }
        ...
        public RMXStream(Stream original, byte[] rnd)
        {

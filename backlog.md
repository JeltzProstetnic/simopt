...
- [ ] **SIM-93** Ivotion KPIs are partly mislabelled and partly unmeasured (P1): `LaborHoursPerSimHour` is assigned a **headcount** (`IvotionKpis.cs:83`) and displayed as "Labor hrs/hr" (`IvotionOptimizationView.axaml.cs:121`) — dimensionally correct only if every operator is busy 100% of the time, which is exactly the utilisation nobody measures. Throughput (`:69`) divides by the **requested** duration rather than `Model.CurrentTime`, applies no warm-up (so a line starting empty under-reports steady state by about one pipeline fill), and uses a single replication with no interval. Floor space and cost are hardcoded arithmetic on the decision vector, never measured. The run driver (`IvotionTopologyHandles.cs:60-71`) reports a `maxSteps`-truncated run as if it completed. (size: M)

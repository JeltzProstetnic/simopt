...
- [ ] **SIM-90** Entity identity is destroyed at every server (P1): `ModelRegistry` leaves `Server`'s default product generator in place, which manufactures a fresh `SimpleEntity` with a **null Identifier** rather than passing the input through. Two consequences — `Buffer.Put` keys on Identifier, so any MCP topology with a server feeding a downstream buffer (i.e. anything multi-stage) fails; and end-to-end cycle time is unmeasurable because the entity reaching the sink is not the one that left the source. The obvious fix (`createProduct: m => m[0]`) **cannot be applied as-is**: `Server` defers the product factory to event-firing time (`Server.cs:840`, `ReturnProduct` at 986) while `InternalFinishedHandler` clears `activeMaterial` (`Server.cs:655`), so the delegate is invoked against an empty list and throws `IndexOutOfRange`. Confirmed empirically 2026-08-26. Fix the lifecycle first, then pass entities through. **Blocks SIM-63's cycle-time collector and every multi-stage MCP model.** (size: M)

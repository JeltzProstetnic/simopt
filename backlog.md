...
- [ ] **SIM-91** First run of a freshly built model diverges from all later runs (P0): measured 476 sink arrivals / 2862 events on run 1 against a stable 475 / 2859 on runs 2 and 3 of the *same* model id, seed 42 (`ModelRegistrySmokeTests.RepeatedRunsOfTheSameModel_ProduceIdenticalResults`). Runs 2+ agree with each other exactly, so the pollution is build-time state that the first `Reset()` does not clear. Leading suspect: `BuildModel` resets with `seedChange` **true** while every later reset runs with it **false**, taking a different branch of `StochasticEntity.Reset` (StochasticEntity.cs:41-49); a second candidate is the source's t=0 entity, created *during* the reset and then destroyed by `Buffer.Reset` on the same pass — if that arrival is being silently discarded it biases every statistic from an auto-starting model. Same defect class as SIM-58, and directly contrary to UN-009 and UN-033. (size: M)

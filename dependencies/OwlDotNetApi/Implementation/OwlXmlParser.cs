...
//			return rNode;
		...
		private void ParseEdgeAttributes(XmlNode node, OwlEdge rEdge)
		{
			...
			for(int i=0;i<count;i++)
			{
				...
				if((!IsSyntacticElement(attr))  || (attr.Prefix == "rdf" && attr.LocalName == "value"))
				{
                    ...
                    /* 17.11.2010 dwi: added literal may contain attribute xml:lang */

					//if the childnode of the edge is a literal then it cant have any arcs going out from it
					if((rEdge.ChildNode != null) && (rEdge.ChildNode is OwlLiteral) && (attr.Name != "rdf:datatype") && (attr.Name != "xml:lang"))
					{
		...
		private OwlNode BuildCollection(XmlNode propertyNode)
		{
//			OwlNode rNode;

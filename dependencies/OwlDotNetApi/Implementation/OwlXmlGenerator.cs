...
namespace OwlDotNetApi
{
	...
	public class OwlXmlGenerator : OwlGenerator, IOwlXmlGenerator
	{
		...
		public void GenerateOwl(IOwlGraph graph, XmlDocument doc)
		{
			...
			//Start with creatig the root
			//XmlDeclaration declaration = _owlDocument.CreateXmlDeclaration("1.0", null, null);
			//_owlDocument.AppendChild(declaration);
			...
			if(parentElement != null) 
			{
				...
				else 
				{
					...
					if(path != _baseUri) 
					{
						...
						if(pos != -1)
						{
							prefix = _namespaces[path].Remove(0,_namespaces[path].Length-pos-1);
						}
					}
				}
		...
		public override void Visit(OwlRestriction node, Object parent) 
		{
			...
			if(parentElement != null) 
			{
				...
				// Attach the element to the its parent

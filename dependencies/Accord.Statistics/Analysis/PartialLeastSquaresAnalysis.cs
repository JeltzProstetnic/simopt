...
namespace Accord.Statistics.Analysis
{
    ...
    public class PartialLeastSquaresAnalysis
    {
        ...
        private double[,] loadingsX;
        private double[,] loadingsY;
        ...
        /// <param name="algorithm">The PLS algorithm to use in the analysis.</param>
        public PartialLeastSquaresAnalysis(double[,] inputs, double[,] outputs, AnalysisMethod method, PartialLeastSquaresAlgorithm algorithm)
        {
            ...
            this.meanX = Tools.Mean(inputs);
            this.meanY = Tools.Mean(outputs);
            this.stdDevX = Tools.StandardDeviation(inputs, meanX);
            this.stdDevY = Tools.StandardDeviation(outputs, meanY);
        }
        #endregion
        ...
        #region Properties
        /// <summary>
        ///   Gets the original data supplied to be analyzied. The first position on the array
        ///   holds the sources for the X input variables, while the second position holds the
        ///   sources for the Y output variables.
        /// </summary>
        public double[][,] Sources
        {
            get { return new double[][,] { sourceX, sourceY }; }
        }

        /// <summary>
        ///   Gets the resulting projection (scores) of the source data into latent space. The
        ///   first position on the array holds the scores for the X input variables, while 
        ///   the second position holds the scores for the Y output variables.
        /// </summary>
        public double[][,] Projections
        {
            get { return new double[][,] { scoresX, scoresY }; }
        }

        /// <summary>
        ///   Gets the column means of the source data.  The first position on the array
        ///   holds the mean vector for the X input variables, while the second position
        ///   holds the mean vector for the Y output variables.
        /// </summary>
        public double[][] Means
        {
            get { return new double[][] { meanX, meanY }; }
        }

        /// <summary>
        ///   Gets the column standard deviations of the source data.  The first position
        ///   on the array holds the standard deviation vector for the X input variables,
        ///   while the second position holds the standard deviations for the Y output variables.
        /// </summary>
        public double[][] StandardDeviations
        {
            get { return new double[][] { stdDevX, stdDevY }; }
        }

        /// <summary>
        ///   Gets the loadings (a.k.a factors or components) obtained during the analysis.
        ///   The first position on the array holds the loadings for the X input variables,
        ///   while the second position holds the loadings for the Y output variables.
        /// </summary>
        public double[][,] FactorMatrix
        {
            get { return new double[][,] { loadingsX, loadingsY }; }
        }
        ...
        /// <summary>
        ///   Gets the ammount of variance explained by each of the PLS factors.
        ///   The first position on the array holds the proportions for the X input factors,
        ///   while the second position holds the proportions for the Y output factors.
        /// </summary>
        public double[][] FactorProportions
        {
            get { return new double[][] { componentProportionX, componentProportionY }; }
        }

        /// <summary>
        ///   Gets the cumulative ammount of variance explained by each of the PLS factors.
        ///   The first position on the array holds the proportions for the X input factors,
        ///   while the second position holds the proportions for the Y output factors.
        /// </summary>
        public double[][] CumulativeProportions
        {
            get { return new double[][] { cumulativeProportionX, cumulativeProportionY }; }
        }
        ...
        public void Compute(int factors)
        {
            ...
            // Creates the object-oriented structure to hold the partial least squares factors
        ...
        /// </summary>
        public double[,] ProjectX(double[,] inputs, int factors)
        {
            int rows = inputs.GetLength(0);
            int cols = inputs.GetLength(1);

            double[,] r = new double[rows, factors];
            double[,] s = adjust(inputs, meanX, stdDevX);
        ...
        /// </summary>
        public double[,] ProjectY(double[,] outputs, int factors)
        {
        ...
        /// </summary>
        protected void nipals(double[,] X0, double[,] Y0, int factors, double tolerance)
        {
            ...
            for (int iteration = 0; iteration < factors && !stop; iteration++)
            {
                ...
                double[] norm_x = Matrix.Norm(X);
                double[] norm_y = Matrix.Norm(Y);
        ...
        ///   Computes PLS parameters using NIPALS algorithm.
        /// </summary>
        protected void simpls(double[,] X0, double[,] Y0, int factors)
        {
        ...
        /// </summary>
        protected double[,] adjust(double[,] m, double[] columnMeans, double[] columnStdDev)
        {
        ...
        /// </summary>
        protected int largest(double[,] m)
        {
            int rows = m.GetLength(0);
            int cols = m.GetLength(1);
    ...
    public class PartialLeastSquaresFactor
    {
        ...
        {
            get { return this.analysis.FactorProportions[0][index]; }
        }
        ...
        {
            get { return this.analysis.CumulativeProportions[0][index]; }
        }
        ...
        {
            get { return this.analysis.FactorProportions[1][index]; }
        }
        ...
        {
            get { return this.analysis.CumulativeProportions[1][index]; }
        }

        /// <summary>
        ///   Gets the latent vectors for this factor. The first position
        ///   contains the independent (input) latent vector and the second
        ///   position contains the dependent (output) latent vector.
        /// </summary>
        public double[][] LatentVectors
        {
            get
            {
                return new double[][] {
                this.analysis.FactorMatrix[0].GetColumn(index),
                this.analysis.FactorMatrix[1].GetColumn(index) };
            }
        }
    }

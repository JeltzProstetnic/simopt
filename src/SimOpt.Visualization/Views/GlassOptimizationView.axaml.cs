...
{
    private GlassOptimizationViewModel? _vm;
    ...
    private void SubscribeToViewModel()
    {
        ...
        _vm = DataContext as GlassOptimizationViewModel;
...
public sealed class GlassSolutionToSummaryConverter : IValueConverter
{
    public static readonly GlassSolutionToSummaryConverter Instance = new();
    public object? Convert(object? value, Type targetType, object? parameter, System.Globalization.CultureInfo culture)
    {
        if (value is not GlassSolution s) return "—";
        return
            $"Mixers          : {s.NumberOfMixers}\n" +
            $"Processing lines: {s.NumberOfProcessingLines}\n" +
            $"Buffer cap      : {s.IntermediateBufferCapacity}\n" +
            $"Ops quality     : {s.OperatorsQuality}\n" +
            $"Ops packing     : {s.OperatorsPacking}";
    }
    public object? ConvertBack(object? value, Type targetType, object? parameter, System.Globalization.CultureInfo culture)
        => throw new NotSupportedException();
}

public sealed class GlassKpisToSummaryConverter : IValueConverter
{
    public static readonly GlassKpisToSummaryConverter Instance = new();
    public object? Convert(object? value, Type targetType, object? parameter, System.Globalization.CultureInfo culture)
    {
        if (value is not GlassKpis k) return "—";
        return

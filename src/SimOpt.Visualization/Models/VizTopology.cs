...
public class VizTopology
{
    ...
    /// <summary>
    /// Glass base-mass (Grundmasse) production line — pilot for the production
    /// simulation PoC. The 5 process steps from the request: raw-material
    /// provisioning → mixing/preparation → intermediate buffer → further
    /// processing → completion. "Weiterverarbeitung" is intentionally the
    /// bottleneck (3.0 min vs a 2.5 min feed), so work-in-process visibly piles
    /// up in the Zwischenlager buffer — the live "Engpass-Früherkennung" demo.
    /// </summary>
    public static VizTopology GlassLine(int seed = 42) => new()
    {
        Name = "Glass Production Line (Base Mass)",
        Seed = seed,
        Nodes = new List<VizNode>
        {
            new() { Id = "raw", Type = "source", Label = "Rohmaterial-\nlager",
                X = 1, Y = 5, Width = 4, Height = 4,
                Params = new() { ["mean_interval"] = 2.5 }, Color = "#66BB6A" },

            new() { Id = "q_mix", Type = "buffer", Label = "Material-\npuffer",
                X = 6.5, Y = 5.5, Width = 2.5, Height = 3,
                Params = new() { ["capacity"] = 20 } },

            new() { Id = "mix", Type = "server", Label = "1. Mischen /\nAufbereitung",
                X = 10.5, Y = 5, Width = 5, Height = 4,
                Params = new() { ["service_time"] = 2.0 }, Color = "#4FC3F7" },

            new() { Id = "wip", Type = "buffer", Label = "Zwischen-\nlager",
                X = 17, Y = 5.5, Width = 3, Height = 3,
                Params = new() { ["capacity"] = 40 } },

            new() { Id = "proc", Type = "server", Label = "2. Weiter-\nverarbeitung",
                X = 22, Y = 5, Width = 5, Height = 4,
                Params = new() { ["service_time"] = 3.0 }, Color = "#FF9800" },

            new() { Id = "done", Type = "sink", Label = "Fertig-\nmeldung",
                X = 28.5, Y = 5, Width = 4, Height = 4,
                Color = "#43A047" },
        },
        Connections = new List<VizConnection>
        {
            new() { From = "raw", To = "q_mix" },
            new() { From = "q_mix", To = "mix" },
            new() { From = "mix", To = "wip" },
            new() { From = "wip", To = "proc" },
            new() { From = "proc", To = "done" },
        }
    };

    /// <summary>
    /// Parametric glass production line. Builds the topology from an optimized
    /// <see cref="SimOpt.Glass.GlassSolution"/>: variable mixer count, variable
    /// processing-line count, chosen intermediate-buffer capacity, and
    /// effective-service-time splits (base_time / operator_count) for the
    /// quality and packing manual stations. Mirrors how
    /// <see cref="IvotionPacking(SimOpt.Ivotion.IvotionSolution,int)"/> varies
    /// node count.
    /// </summary>
    public static VizTopology GlassLine(SimOpt.Glass.GlassSolution sol, int seed = 42)
    {
        int mixers = sol.NumberOfMixers;
        int procLines = sol.NumberOfProcessingLines;
        int bufferCap = sol.IntermediateBufferCapacity;

        // Effective service times for the manual stations (base / operator_count).
        double qualityTime = 2.0 / sol.OperatorsQuality;
        double packingTime = 1.5 / sol.OperatorsPacking;

        var nodes = new List<VizNode>
        {
            new() { Id = "raw", Type = "source", Label = "Rohmaterial-\nlager",
                X = 1, Y = 6, Width = 4, Height = 4,
                Params = new() { ["mean_interval"] = 2.5 }, Color = "#66BB6A" },

            new() { Id = "q_mix", Type = "buffer", Label = "Material-\npuffer",
                X = 6.5, Y = 6.5, Width = 2.5, Height = 3,
                Params = new() { ["capacity"] = 20 } },
        };

        // Variable mixer count (Mischen / Aufbereitung), stacked vertically.
        double mixSpan = 11.0;
        for (int i = 0; i < mixers; i++)
        {
            double y = mixers == 1 ? 6 : 1.5 + i * (mixSpan / mixers);
            nodes.Add(new VizNode
            {
                Id = $"mix{i + 1}", Type = "server",
                Label = $"Mischen /\nAufbereitung {i + 1}",
                X = 10.5, Y = y, Width = 5, Height = 3.5,
                Params = new() { ["service_time"] = 5.0 }, Color = "#4FC3F7",
            });
        }

        // Zwischenlager (intermediate buffer) with chosen capacity.
        nodes.Add(new VizNode
        {
            Id = "wip", Type = "buffer", Label = "Zwischen-\nlager",
            X = 17, Y = 6.5, Width = 3, Height = 3,
            Params = new() { ["capacity"] = bufferCap },
        });

        // Variable processing-line count (Weiterverarbeitung), stacked vertically.
        double procSpan = 11.0;
        for (int i = 0; i < procLines; i++)
        {
            double y = procLines == 1 ? 6 : 1.5 + i * (procSpan / procLines);
            nodes.Add(new VizNode
            {
                Id = $"proc{i + 1}", Type = "server",
                Label = $"Weiter-\nverarbeitung {i + 1}",
                X = 22, Y = y, Width = 5, Height = 3.5,
                Params = new() { ["service_time"] = 6.0 }, Color = "#FF9800",
            });
        }

        nodes.AddRange(new[]
        {
            new VizNode { Id = "q_quality", Type = "buffer", Label = "Quality\nQueue",
                X = 28.5, Y = 6.5, Width = 2.5, Height = 3,
                Params = new() { ["capacity"] = 20 } },
            new VizNode { Id = "quality", Type = "server",
                Label = $"Quality\n({sol.OperatorsQuality} op)",
                X = 32.5, Y = 6, Width = 5, Height = 4,
                Params = new() { ["service_time"] = qualityTime }, Color = "#FFCA28" },

            new VizNode { Id = "q_pack", Type = "buffer", Label = "Packing\nQueue",
                X = 39, Y = 6.5, Width = 2.5, Height = 3,
                Params = new() { ["capacity"] = 15 } },
            new VizNode { Id = "packing", Type = "server",
                Label = $"Packing\n({sol.OperatorsPacking} op)",
                X = 43, Y = 6, Width = 5, Height = 4,
                Params = new() { ["service_time"] = packingTime }, Color = "#AB47BC" },

            new VizNode { Id = "done", Type = "sink", Label = "Fertig-\nmeldung",
                X = 49.5, Y = 6, Width = 4, Height = 4,
                Color = "#43A047" },
        });
        ...
        for (int i = 0; i < mixers; i++)
        {
            connections.Add(new VizConnection { From = "q_mix", To = $"mix{i + 1}" });
            connections.Add(new VizConnection { From = $"mix{i + 1}", To = "wip" });
        }

        for (int i = 0; i < procLines; i++)
        {
            connections.Add(new VizConnection { From = "wip", To = $"proc{i + 1}" });
            connections.Add(new VizConnection { From = $"proc{i + 1}", To = "q_quality" });
        }

        connections.AddRange(new[]
        {
            new VizConnection { From = "q_quality", To = "quality" },
            new VizConnection { From = "quality", To = "q_pack" },
            new VizConnection { From = "q_pack", To = "packing" },
            new VizConnection { From = "packing", To = "done" },
        });

        return new VizTopology
        {
            Name = $"Glass Production Line ({mixers}× mix, {procLines}× proc, buf {bufferCap})",
            Seed = seed,
            Nodes = nodes,
